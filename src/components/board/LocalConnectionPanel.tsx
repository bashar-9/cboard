    ...
    if (networkMode === 'online' && localRoomPrivacy === 'public' && (pairingState === 'joining' || pairingState === 'paired')) {
        return (
            <div className="max-w-xl mx-auto mb-6 rounded-3xl border border-slate-200/70 dark:border-slate-800 bg-white/90 dark:bg-slate-900/80 p-6 shadow-sm">
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            {pairingState === 'paired' ? 'Connected to another device.' : 'Room ready. Open CBoard on the second device.'}
                        </p>
                    </div>
                </div>
                ...
                <p className="text-xs text-slate-400 mt-4 text-center">Anyone using this domain on the same internet connection joins automatically.</p>
            </div>
        );
    }
