...
export function HowItWorks({ className }: { className?: string }) {
    return (
        <motion.div
            ...
            initial="hidden"
            animate="visible"
            className={`flex flex-col items-center justify-center px-4 max-w-2xl mx-auto select-none ${className || 'py-16 sm:py-24'}`}
        >
            {/* Header */}
                    ...
                    >
                        ...
                        <div className="relative flex items-start gap-5 p-5 sm:p-6 rounded-[1.5rem] bg-white/80 dark:bg-slate-900/60 backdrop-blur-2xl border border-white/60 dark:border-slate-800/60 shadow-[0_4px_24px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_24px_rgb(0,0,0,0.15)] transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)]">
                            ...
                            <div className="flex-1 min-w-0 pt-0.5">
                                <h3 className="text-[15px] font-semibold text-slate-800 dark:text-slate-100 tracking-tight">
                                    {step.title}
                                </h3>
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-3">
                {features.map((f) => (
                    <div
                        key={f.label}
                        className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 shadow-sm transition-colors"
                    >
            ...
            {/* Hint */}
            <motion.p variants={itemVariants} className="text-xs text-slate-400 dark:text-slate-600 mt-10 text-center">
                Use the input bar below to share your first item ↓
            </motion.p>
        </motion.div>
    );
}
